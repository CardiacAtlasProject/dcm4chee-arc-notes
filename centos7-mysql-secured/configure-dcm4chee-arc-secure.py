#!/usr/bin/python
"""Configure dcm4chee-arc secured ui. You must run the dcm4chee-arc server.

Usage:
    configure-dcm4chee-arc-secure.py <configFile>
    configure-dcm4chee-arc-secure.py -h
...
if __name__ == "__main__":
    ...
    with open('/tmp/attribs.cli','w') as f:
    ...
    f.close()
    subprocess.call([jbossCLI, '-c', '--file=/tmp/attribs.cli'])
    ...
    # cleaning up
    for f in ['add-data-source.cli', 'attribs.cli', 'deploy.cli']:
        os.remove('/tmp/' + f)
